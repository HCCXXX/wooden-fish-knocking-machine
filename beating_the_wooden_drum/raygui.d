...
static if (!HasVersion!"RAYGUI_NO_ICONS" && !HasVersion!"RAYGUI_CUSTOM_ICONS") {
	...
	// guiIcons size is by default: 256*(16*16/32) = 2048*4 = 8192 bytes = 8 KB
	//----------------------------------------------------------------------------------
	private uint[RAYGUI_ICON_MAX_ICONS*RAYGUI_ICON_DATA_ELEMENTS] guiIcons = [
...
// guiStyle size is by default: 16*(16 + 8) = 384*4 = 1536 bytes = 1.5 KB
//----------------------------------------------------------------------------------
private uint[RAYGUI_MAX_CONTROLS*(RAYGUI_MAX_PROPS_BASE + RAYGUI_MAX_PROPS_EXTENDED)] guiStyle = 0;

private bool guiStyleLoaded = false;         // Style loaded flag for lazy style initialization
...
void GuiEnable() { if (guiState == STATE_DISABLED) guiState = STATE_NORMAL; }

// Disable gui global state
// NOTE: We check for STATE_NORMAL to avoid messing custom global state setups
void GuiDisable() { if (guiState == STATE_NORMAL) guiState = STATE_DISABLED; }
...
void GuiSetState(int state) { guiState = cast(GuiState)state; }

// Get gui state (global state)
int GuiGetState() { return guiState; }
...
bool GuiWindowBox(Rectangle bounds, const(char)* title) {
    ...
    Rectangle statusBar = { bounds.x, bounds.y, bounds.width, cast(float)statusBarHeight };
    if (bounds.height < statusBarHeight*2.0f) bounds.height = statusBarHeight*2.0f;

    Rectangle windowPanel = { bounds.x, bounds.y + cast(float)statusBarHeight - 1, bounds.width, bounds.height - cast(float)statusBarHeight + 1 };
...
void GuiLine(Rectangle bounds, const(char)* text)
{
    ...
    else
    {
        ...
        GuiDrawRectangle(Rectangle( bounds.x, bounds.y + bounds.height/2, RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 ), 0, Colors.BLANK, color);
...
void GuiPanel(Rectangle bounds, const(char)* text) {
    ...
    Rectangle statusBar = { bounds.x, bounds.y, bounds.width, cast(float)RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT };
...
Rectangle GuiScrollPanel(Rectangle bounds, const(char)* text, Rectangle content, Vector2* scroll)
{
    ...
    if (hasHorizontalScrollBar)
    {
        ...
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, cast(int)(((bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth)/cast(int)content.width)*(cast(int)bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth)));
        scrollPos.x = cast(float)-GuiScrollBar(horizontalScrollBar, cast(int)-scrollPos.x, cast(int)horizontalMin, cast(int)horizontalMax);
    }
    ...
    if (hasVerticalScrollBar)
    {
        ...
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, cast(int)(((bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth)/cast(int)content.height)*(cast(int)bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth)));
        scrollPos.y = cast(float)-GuiScrollBar(verticalScrollBar, cast(int)-scrollPos.y, cast(int)verticalMin, cast(int)verticalMax);
    }
...
int GuiToggleGroup(Rectangle bounds, const(char)* text, int active) {
    ...
    int itemCount = 0;
    const(char)** items = GuiTextSplit(text, ';', &itemCount, rows.ptr);
...
int GuiComboBox(Rectangle bounds, const(char)* text, int active) {
    ...
    Rectangle selector = { cast(float)bounds.x + bounds.width + GuiGetStyle(COMBOBOX, COMBO_BUTTON_SPACING),
	cast(float)bounds.y, cast(float)GuiGetStyle(COMBOBOX, COMBO_BUTTON_WIDTH), cast(float)bounds.height };
...
bool GuiValueBox(Rectangle bounds, const(char)* text, int* value, int minValue, int maxValue, bool editMode) {
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (editMode)
        {
            ...
            int keyCount = cast(int)strlen(textValue.ptr);

            // Only allow keys in range [48..57]
            if (keyCount < RAYGUI_VALUEBOX_MAX_CHARS)
            {
                if (GetTextWidth(textValue.ptr) < bounds.width)
                {
                    int key = GetCharPressed();
                    if ((key >= 48) && (key <= 57))
                    {
                        textValue[keyCount] = cast(char)key;
                        keyCount++;
            ...
            if (valueHasChanged) *value = TextToInteger(textValue.ptr);
...
float GuiSliderPro(Rectangle bounds, const(char)* textLeft, const(char)* textRight, float value, float minValue, float maxValue, int sliderWidth) {
    ...
    if (sliderWidth > 0)        // Slider
    {
        ...
        slider.width = cast(float)sliderWidth;
    }
    else if (sliderWidth == 0)  // SliderBar
    {
        slider.x += GuiGetStyle(SLIDER, BORDER_WIDTH);
        slider.width = cast(float)sliderValue;
    }
...
int GuiListViewEx(Rectangle bounds, const(char)** text, int count, int* focus, int* scrollIndex, int active) {
    ...
    itemBounds.height = cast(float)GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT);
    if (useScrollBar) itemBounds.width -= GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH);

    // Get items on the list
    int visibleItems = cast(int)bounds.height/(GuiGetStyle(LISTVIEW, LIST_ITEMS_HEIGHT) + GuiGetStyle(LISTVIEW, LIST_ITEMS_SPACING));
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (useScrollBar)
            {
                int wheelMove = cast(int)GetMouseWheelMove();
    ...
    if (useScrollBar)
    {
        Rectangle scrollBarBounds = {
            ...
            bounds.y + GuiGetStyle(LISTVIEW, BORDER_WIDTH), cast(float)GuiGetStyle(LISTVIEW, SCROLLBAR_WIDTH),
            bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH)
        };

        // Calculate percentage of visible items and apply same percentage to scrollbar
        float percentVisible = cast(float)(endIndex - startIndex)/count;
        float sliderSize = bounds.height*percentVisible;

        int prevSliderSize = GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE);   // Save default slider size
        int prevScrollSpeed = GuiGetStyle(SCROLLBAR, SCROLL_SPEED); // Save default scroll speed
        GuiSetStyle(SCROLLBAR, SCROLL_SLIDER_SIZE, cast(int)sliderSize);            // Change slider size
...
Color GuiColorPanel(Rectangle bounds, const(char)* text, Color color)
{
    ...
    pickerSelector.x = bounds.x + cast(float)hsv.y*bounds.width;            // HSV: Saturation
    pickerSelector.y = bounds.y + (1.0f - cast(float)hsv.z)*bounds.height;  // HSV: Value
...
float GuiColorBarAlpha(Rectangle bounds, const(char)* text, float alpha) {
    ...
    if (state != STATE_DISABLED)
    {
        int checksX = cast(int)bounds.width/RAYGUI_COLORBARALPHA_CHECKED_SIZE;
        int checksY = cast(int)bounds.height/RAYGUI_COLORBARALPHA_CHECKED_SIZE;
...
float GuiColorBarHue(Rectangle bounds, const(char)* text, float hue) {
    ...
    if (state != STATE_DISABLED)
    {
        ...
        DrawRectangleGradientV(cast(int)bounds.x, cast(int)(bounds.y + 4*(bounds.height/6)), cast(int)bounds.width, cast(int)ceilf(bounds.height/6), Fade(Color( 0, 0, 255, 255 ), guiAlpha), Fade(Color( 255, 0, 255, 255 ), guiAlpha));
        DrawRectangleGradientV(cast(int)bounds.x, cast(int)(bounds.y + 5*(bounds.height/6)), cast(int)bounds.width, cast(int)(bounds.height/6), Fade(Color( 255, 0, 255, 255 ), guiAlpha), Fade(Color( 255, 0, 0, 255 ), guiAlpha));
    }
...
void GuiLoadStyle(const(char)* fileName) {
    ...
    if (rgsFile != null)
    {
        char[MAX_LINE_BUFFER_SIZE] buffer = 0;
        fgets(buffer.ptr, MAX_LINE_BUFFER_SIZE, rgsFile);
                    ...
                    case 'p':
						{
							...
							sscanf(buffer.ptr, "p %d %d 0x%x", &controlId, &propertyId, &propertyValue);
							GuiSetStyle(controlId, propertyId, cast(int)propertyValue);
...
const(char)* GuiIconText(int iconId, const(char)* text) {
	version (RAYGUI_NO_ICONS) {
		...
		static char[1024] buffer = 0;
		static char[6] iconBuffer = 0;

		if (text != null)
		{
			memset(buffer.ptr, 0, 1024);
			sprintf(buffer.ptr, "#%03i#", iconId);
		...
		{
			sprintf(iconBuffer.ptr, "#%03i#", iconId & 0x1ff);

			return iconBuffer.ptr;
		}
	}
}
...
static if (!HasVersion!"RAYGUI_NO_ICONS") {
	...
	uint* GuiGetIcons() { return guiIconsPtr; }
	...
	char** GuiLoadIcons(const(char)* fileName, bool loadIconsName) {
		...
		if (rgiFile != null)
		{
				...
				(signature[3] == ' '))
			{
				if (loadIconsName)
				{
					guiIconsName = cast(char**)RAYGUI_MALLOC(iconCount*(char**).sizeof);
...
private int GetTextWidth(const(char)* text) {
    ...
    if ((text != null) && (text[0] != '\0'))
    {
        ...
        float fontSize = cast(float)GuiGetStyle(DEFAULT, TEXT_SIZE);
        ...
        if ((guiFont.texture.id > 0) && (text != null))
        {
            ...
            float scaleFactor = fontSize/cast(float)guiFont.baseSize;
            textSize.y = cast(float)guiFont.baseSize*scaleFactor;
...
private const(char)* GetTextIcon(const(char)* text, int* iconId) {
	static if (!HasVersion!"RAYGUI_NO_ICONS") {
		*iconId = -1;
		...
		{
			char[4] iconValue = 0;  // Maximum length for icon value: 3 digits + '\0'
			...
			if (text[pos] == '#')
			{
				*iconId = TextToInteger(iconValue.ptr);
